...
def params(Survey1='MK_UHF',Survey2=None,A_sky1=None,A_sky2=None,A_skyX=None,zminzmax=None,f_tobsloss=0,t_tot=0):
    ...
    if Survey1=='DESI-LRG' or Survey2=='DESI-LRG': # DESI Luminous Red Galaxies
        ...
        if A_sky2==None: A_sky2 = 7500 # area in sq.deg
        A_skys.append(A_sky2) # area in sq.deg
        ####### AMEND TO N(z) profile ##########
        ...
        if A_sky2==None: A_sky2 = 18000 # area in sq.deg
        A_skys.append(A_sky2) # area in sq.deg
        Ngal_per_deg2 = 172800 # 48arcmin^-2 Y10 sample (https://arxiv.org/pdf/1809.01669 pg 48)
        Ngal = Ngal_per_deg2 * A_skys[-1]
        nbar = Ngal / Vsur(zmins[-1],zmaxs[-1],A_skys[-1])
        z = np.mean([np.max(zmins),np.min(zmaxs)]) # use single z_eff from all surveys for photoz error
        sigma_z2 = 0.02*(1+z) # LSST Gold Sample see Sec 3.8.1 (https://arxiv.org/pdf/0912.0201)
    elif Survey1=='Rubin_early' or Survey2=='Rubin_early': # as above just with half the number density and 10,000deg2
        if zminzmax==None: zmins.append(0.3);zmaxs.append(3)
        else: zmins.append(zminzmax[0]); zmaxs.append(zminzmax[1])
        if A_sky2==None: A_sky2 = 10000 # area in sq.deg
        A_skys.append(A_sky2) # area in sq.deg
        Ngal_per_deg2 = 100000 # roughly half the density
        Ngal = Ngal_per_deg2 * A_skys[-1]
        nbar = Ngal / Vsur(zmins[-1],zmaxs[-1],A_skys[-1])
        z = np.mean([np.max(zmins),np.min(zmaxs)]) # use single z_eff from all surveys for photoz error
        sigma_z2 = 0.02*(1+z) # LSST Gold Sample see Sec 3.8.1 (https://arxiv.org/pdf/0912.0201)
    ...
    if sigma_z1 is None: sigma_z1 = 0
    if sigma_z2 is None: sigma_z2 = 0
    theta_FWHM2 = 0 # currently always assume no beam in survey 2 (i.e. galaxy survey)
